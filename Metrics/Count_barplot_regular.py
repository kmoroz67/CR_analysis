...
def calc_number_of_clones_groups(samples, chain):
    ...
    Calc Hyperexpanded, Large, Medium, Small, Rare number of clones groups for required samples.
    ...
    :return : cn_rare, cn_small, cn_med, cn_large, cn_hyp
    ...
    for samp in samples:
        ...
        number_of_clones['Rare'] += data[(data.fraction <=
                                          0.00001)].cdr3aa.nunique()
        ...
        number_of_clones['Hyperexpanded'] += data[(data.fraction >
                                                   0.01)].cdr3aa.nunique()
    ...
    for i in range(len(samples)):
        summ = cn_rare[i] + cn_small[i] + cn_med[i] + cn_large[i] + cn_hyp[i]
...
def count_healthy_cohort(chain, material_type, healthy_base, seq_type):
    '''
    ...
    :return : h_cn_rare, h_cn_small, h_cn_med, h_cn_large, h_cn_hyp
    ...
    h_cn_rare, h_cn_small, h_cn_med, h_cn_large, h_cn_hyp = calc_number_of_clones_groups(
...
def number_of_clones_barplot(samples,
    ...
    cn_rare, cn_small, cn_med, cn_large, cn_hyp = calc_number_of_clones_groups(
        samples, chain)
    # if 'healthy' == True calculation fraction groups for healthy cohort
    if healthy:
        names.append('Healthy donors')
        h_cn_rare, h_cn_small, h_cn_med, h_cn_large, h_cn_hyp = count_healthy_cohort(
